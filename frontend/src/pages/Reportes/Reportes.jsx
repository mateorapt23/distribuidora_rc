...
export function generarMeses() {
  ...
  for (let i = 0; i < 13; i++) {
    ...
    const desde = `${year}-${String(month + 1).padStart(2, '00')}-01`;
    const ultimo = new Date(year, month + 1, 0);
    const hasta  = `${year}-${String(month + 1).padStart(2, '00')}-${String(ultimo.getDate()).padStart(2, '0')}`;
...
const MonthPicker = ({ meses, value, onChange }) => {
  ...
  // Panel anchored to left on mobile (avoids going off-screen to the right)
  const panelStyle = {
    position: 'absolute',
    top: 'calc(100% + 8px)',
    left: isSmall ? 0 : 'auto',
...
export default function Reportes() {
  ...
  const pad = isSmall ? '14px 16px' : '0 28px';

  if (!esAdmin) return (
    <div style={{ background: C.bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: C.card, borderRadius: 24, padding: isSmall ? '32px 20px' : '52px 44px', maxWidth: 440, width: '100%',
  ...
  const tabs = [
    { key: 'ventas',      label: 'Ventas',                 shortLabel: 'Ventas',     icon: <IcoVentas /> },
    { key: 'movimientos', label: 'Movimientos de stock',   shortLabel: 'Movimientos', icon: <IcoMovimientos /> },
    { key: 'productos',   label: 'Productos más vendidos', shortLabel: 'Productos',   icon: <IcoProductos /> },
  ];
    ...
    <div style={{ background: C.bg, minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            <div style={{ fontSize: isSmall ? 18 : 22, fontWeight: 700, color: C.textPrimary }}>Reportes</div>
        ...
        {/* Selector de mes — full width en móvil */}
        <div style={{ width: isSmall ? '100%' : 'auto' }}>
          <MonthPicker
            meses={MESES}
      ...
      <div style={{
        ...
        borderBottom: `1px solid ${C.border}`,
        background: '#fff',
        padding: isSmall ? '0 12px' : '0 28px',
        overflowX: 'auto',
        WebkitOverflowScrolling: 'touch',
        // Esconder scrollbar manteniendo funcionalidad
        ...
        {tabs.map(tab => (
          <button key={tab.key} onClick={() => setSeccion(tab.key)}
            style={{
              ...
              fontSize: isSmall ? 12 : 13, fontWeight: 600,
              display: 'flex', alignItems: 'center', gap: isSmall ? 5 : 7,
            ...
            {tab.icon}
            {isSmall ? tab.shortLabel : tab.label}
          </button>
        ))}
      </div>
